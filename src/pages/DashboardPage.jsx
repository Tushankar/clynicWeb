  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-5 lg:grid-cols-[1.4fr_1fr]">
        ...
        <Card className="card-lift grid grid-cols-1 gap-4 p-5 sm:grid-cols-3">
          <div>
            <div className="flex items-center gap-1.5 text-xs font-medium text-muted-foreground"><CalendarCheck weight="duotone" className="h-4 w-4" /> Today</div>
            <p className="mt-1.5 text-sm font-semibold leading-snug text-foreground">{dateLabel}</p>
            <p className="mt-0.5 text-xs text-muted-foreground">Kolkata, India</p>
          </div>
          <div className="sm:border-l sm:pl-4">
            <p className="text-xs font-medium text-muted-foreground">Clinic status</p>
            <span className="mt-1.5 inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-2.5 py-1 text-xs font-semibold text-emerald-600">
          ...
          <div className="sm:border-l sm:pl-4">
            ...
            {nextAppt ? (
              <>
                <p className="mt-1.5 text-lg font-bold tabular text-foreground">{fmtTime(nextAppt.scheduledAt)}</p>
                <p className="truncate text-xs text-muted-foreground">{nextAppt.patientName}</p>
                <span className="mt-1 inline-flex items-center gap-1 text-xs font-medium text-primary"><ClockCountdown weight="duotone" className="h-3.5 w-3.5" /> in {minutesTo} min</span>
              </>
            ) : (
              <p className="mt-1.5 text-sm text-muted-foreground">Nothing upcoming</p>
            )}
          </div>
        </Card>
      </div>
      ...
      <div className="grid gap-5 xl:grid-cols-12">
        ...
        <SectionCard className="xl:col-span-4" title="Today's Appointments" action={<ViewAll to="/dashboard/appointments" />} bodyClassName="p-0">
          {appts.length === 0 ? (
        ...
        {/* AI assistant */}
        <SectionCard className="xl:col-span-3" title="AI Assistant" icon={Sparkle}>
